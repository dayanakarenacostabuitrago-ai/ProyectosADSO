                        ...
                        <div class="sb-main">
                            ...
                            <div class="page-card p-0 overflow-hidden">
                                ...
                                <div class="p-3">
                                    ...
                                    <div style="background:#e8f2ee;border-radius:12px;padding:.9rem 1rem;border:1px solid #ccddd5;margin-bottom:1rem;">
                                        <form method="get" action="${pageContext.request.contextPath}/citas" class="d-flex gap-2 align-items-end flex-wrap">
                                            <input type="hidden" name="accion" value="listar"/>
                                            <div>
                                                <label style="font-size:.76rem;font-weight:700;color:#4a6258;display:block;margin-bottom:.3rem;text-transform:uppercase;letter-spacing:.04em;">
                                                    <i class="fas fa-stethoscope me-1"></i><fmt:message key="cita.filter.specialty"/>
                                                </label>
                                                <select name="filtroEspecialidad" class="form-select form-select-sm" style="min-width:160px;border-radius:8px;border-color:#ccddd5;">
                                                    <option value=""><fmt:message key="cita.filter.all"/></option>
                                                    <c:forEach var="esp" items="${especialidades}">
                                                        <option value="${esp.idEspecialidad}" ${filtroEspecialidad == esp.idEspecialidad.toString() ? 'selected' : ''}>${esp.nombre}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                            <div>
                                                <label style="font-size:.76rem;font-weight:700;color:#4a6258;display:block;margin-bottom:.3rem;text-transform:uppercase;letter-spacing:.04em;">
                                                    <i class="fas fa-tag me-1"></i><fmt:message key="cita.filter.status"/>
                                                </label>
                                                <select name="filtroEstado" class="form-select form-select-sm" style="min-width:150px;border-radius:8px;border-color:#ccddd5;">
                                                    <option value=""><fmt:message key="cita.filter.all"/></option>
                                            ...
                                            <button type="submit" class="btn btn-success btn-sm rounded-pill px-3" style="height:31px;">
                                                <i class="fas fa-filter me-1"></i><fmt:message key="cita.filter.apply"/>
                                            </button>
                                            <a href="${pageContext.request.contextPath}/citas?accion=listar" class="btn btn-outline-secondary btn-sm rounded-pill" style="height:31px;line-height:1.6;">
                                                <i class="fas fa-times me-1"></i><fmt:message key="cita.filter.clear"/>
                                            </a>
                                            <span style="font-size:.78rem;color:#4a6258;align-self:center;margin-left:auto;">
