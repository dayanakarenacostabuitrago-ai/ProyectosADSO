...
<%@page import="java.util.List, karen.adso.biblioteca.modelo.Categoria, karen.adso.biblioteca.modelo.Usuario"%>
<%
...
<html lang="es">
    <head>
        ...
        <title>Categorías — Biblioteca SENA</title>
    ...
    <body class="panel-body" style="background:var(--bg)">
        ...
        <aside class="sb">
            ...
            <a href="${pageContext.request.contextPath}/CategoriaServlet" class="ni act" title="Categorías"><i class="fas fa-tags"></i><span>Categ.</span></a>
                    <% } %><% }%>
            <div class="sb-bot">
        ...
        <main class="main-content">
            ...
            <div class="page-header">
                <div>
                    <div class="page-title"><span>Categorías</span> de Libros</div>
                    <div class="page-subtitle">Clasificación temática del catálogo bibliográfico</div>
                </div>
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <% if (esAdmin) { %>
                    <a href="${pageContext.request.contextPath}/CategoriaServlet?accion=nuevo" class="btn-panel-gold no-print">
            ...
            <div class="row g-3 mb-4">
                <div class="col-sm-4">
                    <div class="stat-card">
                        <div class="stat-icon" style="background:rgba(66,165,245,.15)"><i class="fas fa-tags" style="color:var(--b4)"></i></div>
                        <div class="stat-value"><%=categorias != null ? categorias.size() : 0%></div>
            ...
            <div class="row g-3 mb-4" id="gridCategorias">
                <%
                    String[] colores = {"#42A5F5", "#FFD54F", "#4ade80", "#f87171", "#c084fc", "#fb923c"};
                    String[] iconos = {"fa-bookmark", "fa-star", "fa-heart", "fa-fire", "fa-gem", "fa-magic"};
                    int ci = 0;
                    if (categorias != null && !categorias.isEmpty()) {
                        for (Categoria c : categorias) {
                            String col = colores[ci % colores.length];
                            String ico = iconos[ci % iconos.length];
                            ci++;
                %>
                <div class="col-sm-6 col-lg-4 categoria-item">
                    <div class="stat-card d-flex align-items-center gap-3">
                        <div class="stat-icon" style="background:rgba(100,100,255,.12);flex-shrink:0">
                            <i class="fas <%=ico%>" style="color:<%=col%>"></i>
                        </div>
                        <div style="flex:1">
                            <div style="font-weight:700;color:var(--blanco)"><%=c.getNombre()%></div>
                            <div style="font-size:.78rem;color:rgba(255,255,255,.4)">-<%=c.getDescripcion()%></div>
                        </div>
                        <% if (esAdmin) {%>
                        <div class="d-flex gap-1 no-print">
                            <a href="${pageContext.request.contextPath}/CategoriaServlet?accion=editar&id=<%=c.getId()%>"
                               class="btn-sm-action btn-edit" title="Editar"><i class="fas fa-pen"></i></a>
                            <a href="${pageContext.request.contextPath}/CategoriaServlet?accion=eliminar&id=<%=c.getId()%>"
                               class="btn-sm-action btn-delete" title="Eliminar"
                               onclick="return confirm('¿Eliminar la categoría <%=c.getNombre()%>?')">
            ...
            <table id="tablaCategoriasExport" style="display:none">
                <thead><tr><th>ID</th><th>Nombre</th></tr></thead>
                <tbody>
                    <% if (categorias != null)
                            for (Categoria c : categorias) {%>
                    <tr><td><%=c.getId()%></td><td><%=c.getNombre()%></td></tr>
                    <% }%>
                </tbody>
            </table>
        </main>
        ...
        <script>
            function exportarPDF() {
                ...
                doc.text('Categorías — Biblioteca SENA', 14, 18);
                doc.setFontSize(9);
                doc.setTextColor(120, 120, 120);
                doc.text('Generado: ' + new Date().toLocaleString('es-CO'), 14, 24);
                const filas = [];
                document.querySelectorAll('#tablaCategoriasExport tbody tr').forEach(r => {
                    const c = r.querySelectorAll('td');
                    filas.push([c[0].textContent.trim(), c[1].textContent.trim()]);
                });
                doc.autoTable({
                    startY: 28,
                    head: [['ID', 'Nombre de Categoría']],
                    body: filas,
                    styles: {fontSize: 10, cellPadding: 4},
                    headStyles: {fillColor: [21, 101, 192], textColor: 255, fontStyle: 'bold'},
                    alternateRowStyles: {fillColor: [245, 248, 255]}
                });
                doc.save('categorias_biblioteca.pdf');
            }

            function exportarExcel() {
                const filas = [['ID', 'Nombre de Categoría']];
                document.querySelectorAll('#tablaCategoriasExport tbody tr').forEach(r => {
                    const c = r.querySelectorAll('td');
                    filas.push([c[0].textContent.trim(), c[1].textContent.trim()]);
                });
                ...
                ws['!cols'] = [{wch: 8}, {wch: 35}];
                XLSX.utils.book_append_sheet(wb, ws, 'Categorías');
                XLSX.writeFile(wb, 'categorias_biblioteca.xlsx');
            }
        </script>
