...
<%@page import="java.util.List, karen.adso.biblioteca.modelo.Autor, karen.adso.biblioteca.modelo.Usuario"%>
<%
...
<html lang="es">
    <head>
        ...
        <title>Autores — Biblioteca SENA</title>
    ...
    <body class="panel-body" style="background:var(--bg)">
        ...
        <main class="main-content">
            ...
            <div class="page-header">
                <div>
                    <div class="page-title"><span>Autores</span> Registrados</div>
                    <div class="page-subtitle">Gestión del catálogo de autores literarios</div>
                </div>
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <% if (esAdmin) { %>
                    <a href="${pageContext.request.contextPath}/AutorServlet?accion=nuevo" class="btn-panel-gold no-print">
            ...
            <div class="row g-3 mb-4">
                <div class="col-sm-4">
                    <div class="stat-card">
                        <div class="stat-icon" style="background:rgba(66,165,245,.15)"><i class="fas fa-feather-alt" style="color:var(--b4)"></i></div>
                        <div class="stat-value"><%=autores != null ? autores.size() : 0%></div>
            ...
            <div class="panel-card">
                <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2 no-print">
                    <h6 class="mb-0" style="color:var(--b4);font-weight:700"><i class="fas fa-list me-2"></i>Lista de Autores</h6>
                    <div class="search-box" style="width:260px">
                        <i class="fas fa-search"></i>
                        <input type="text" id="searchInput" placeholder="Buscar autor...">
                    </div>
                </div>

                <div class="table-responsive">
                    <table class="table-panel" id="tablaAutores">
                        <thead>
                            <tr>
                                <th>#</th>
                        ...
                        <tbody>
                            <% if (autores != null && !autores.isEmpty()) {
                                    for (Autor a : autores) {%>
                            <tr>
                                <td><span style="color:rgba(255,255,255,.35);font-size:.8rem"><%=a.getId()%></span></td>
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <div style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#1565C0,#42A5F5);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.85rem;flex-shrink:0">
                                            <%=a.getNombre().charAt(0)%>
                                        </div>
                                        <span style="font-weight:600"><%=a.getNombre()%> <%=a.getApellido()%></span>
                                    </div>
                                </td>
                                <td style="color:rgba(255,255,255,.6)">
                                    <i class="fas fa-globe-americas me-1" style="color:var(--b4);font-size:.75rem"></i>
                                    <%=a.getNacionalidad() != null ? a.getNacionalidad() : "—"%>
                                </td>
                                <td style="color:rgba(255,255,255,.6)"><%=a.getFechaNacimiento() != null ? a.getFechaNacimiento().toString() : "—"%></td>
                                <% if (esAdmin) {%>
                                <td style="text-align:center" class="no-print">
                                    <div class="d-flex gap-2 justify-content-center">
                                        <a href="${pageContext.request.contextPath}/AutorServlet?accion=editar&id=<%=a.getId()%>"
                                           class="btn-sm-action btn-edit" title="Editar"><i class="fas fa-pen"></i></a>
                                        <a href="${pageContext.request.contextPath}/AutorServlet?accion=eliminar&id=<%=a.getId()%>"
                                           class="btn-sm-action btn-delete" title="Eliminar"
                                           onclick="return confirm('¿Eliminar al autor <%=a.getNombre()%> <%=a.getApellido()%>?')">
        ...
        <script>
            document.getElementById('searchInput').addEventListener('input', function () {
                ...
                document.querySelectorAll('#tablaAutores tbody tr').forEach(r => {
            ...
            function exportarPDF() {
                ...
                doc.text('Autores — Biblioteca SENA', 14, 18);
                doc.setFontSize(9);
                ...
                doc.autoTable({
                    startY: 28,
                    head: [['#', 'Nombre Completo', 'Nacionalidad', 'Fecha Nacimiento']],
                    body: filas,
                    styles: {fontSize: 9, cellPadding: 3},
                    headStyles: {fillColor: [21, 101, 192], textColor: 255, fontStyle: 'bold'},
                    alternateRowStyles: {fillColor: [245, 248, 255]}
                });
                doc.save('autores_biblioteca.pdf');
            }

            function exportarExcel() {
                const filas = [['ID', 'Nombre Completo', 'Nacionalidad', 'Fecha Nacimiento']];
                document.querySelectorAll('#tablaAutores tbody tr').forEach(r => {
                    if (r.style.display === 'none') return;
                    const c = r.querySelectorAll('td');
                    if (c.length < 4) return;
                ...
                ws['!cols'] = [{wch: 6}, {wch: 35}, {wch: 18}, {wch: 18}];
                XLSX.utils.book_append_sheet(wb, ws, 'Autores');
                XLSX.writeFile(wb, 'autores_biblioteca.xlsx');
            }
        </script>
