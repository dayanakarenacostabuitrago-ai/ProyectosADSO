...
<title>${accion == 'create' ? 'Nuevo' : 'Editar'} Documento - Registraduria</title>
...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="${pageContext.request.contextPath}/ciudadanos" class="nav-item"><i class="fas fa-user-circle"></i> Ciudadanos</a>
    <a href="${pageContext.request.contextPath}/documentos" class="nav-item active"><i class="fas fa-id-card"></i> Documentos</a>
...
<div class="main">
  <div class="topbar">
    <a class="back" href="${pageContext.request.contextPath}/documentos"><i class="fas fa-arrow-left"></i> Volver al listado</a>
    <span style="color:#d1dbe4;">|</span>
    <h1>
      <i class="fas ${accion == 'create' ? 'fa-file-plus' : 'fa-file-pen'}" style="color:#c8a84b;"></i>
      ${accion == 'create' ? 'Nuevo Documento' : 'Editar Documento'}
    </h1>
  </div>
  ...
  <div class="content">
    <div class="form-card">
      <div class="form-card-header">
        <i class="fas ${accion == 'create' ? 'fa-file-plus' : 'fa-file-pen'}"></i>
        <h2>${accion == 'create' ? 'Registrar nuevo documento' : 'Actualizar documento expedido'}</h2>
      </div>
      <div class="form-card-body">
        ...
        <form action="${pageContext.request.contextPath}/documentos" method="post">
          ...
          <c:if test="${accion == 'update'}">
            <input type="hidden" name="id" value="${documento.idDocumentosExpedidos}">
          </c:if>
          <div class="mb-3">
            <label class="form-label fw-semibold" style="font-size:12px;text-transform:uppercase;color:#5a6a7a;">Documento del Ciudadano *</label>
            <input type="text" name="ciudadanoDocumento" class="form-control" value="${documento.ciudadanoDocumento}" required ${accion == 'update' ? 'readonly' : ''} style="${accion == 'update' ? 'background:#f7f9fc;color:#888;' : ''}">
          </div>
          <div class="row g-3 mb-3">
            <div class="col-md-6">
              <label class="form-label fw-semibold" style="font-size:12px;text-transform:uppercase;color:#5a6a7a;">Tipo de Documento *</label>
              <select name="tipoDocumento" class="form-select" required>
                <option value="">Seleccionar...</option>
                <option ${documento.tipoDocumento == 'Cedula de Ciudadania' ? 'selected' : ''}>Cedula de Ciudadania</option>
                <option ${documento.tipoDocumento == 'Tarjeta de Identidad' ? 'selected' : ''}>Tarjeta de Identidad</option>
                <option ${documento.tipoDocumento == 'Registro Civil' ? 'selected' : ''}>Registro Civil</option>
                <option ${documento.tipoDocumento == 'Contrasenha' ? 'selected' : ''}>Contrasenha</option>
              </select>
            </div>
            <div class="col-md-6">
              <label class="form-label fw-semibold" style="font-size:12px;text-transform:uppercase;color:#5a6a7a;">Numero de Serie *</label>
              <input type="text" name="numeroSerie" class="form-control" value="${documento.numeroSerie}" required>
            </div>
          </div>
          <div class="row g-3 mb-3">
            <div class="col-md-6">
              <label class="form-label fw-semibold" style="font-size:12px;text-transform:uppercase;color:#5a6a7a;">Fecha de Expedicion *</label>
              <input type="date" name="fechaExpedicion" class="form-control" value="${documento.fechaExpedicion}" required>
            </div>
            <div class="col-md-6">
              <label class="form-label fw-semibold" style="font-size:12px;text-transform:uppercase;color:#5a6a7a;">Fecha de Vencimiento</label>
              <input type="date" name="fechaVencimiento" class="form-control" value="${documento.fechaVencimiento}">
            </div>
          </div>
          <div class="mb-4">
            <label class="form-label fw-semibold" style="font-size:12px;text-transform:uppercase;color:#5a6a7a;">Estado *</label>
            <select name="estado" class="form-select" required>
              <option value="vigente" ${documento.estado == 'vigente' ? 'selected' : ''}>Vigente</option>
              <option value="vencido" ${documento.estado == 'vencido' ? 'selected' : ''}>Vencido</option>
              <option value="cancelado" ${documento.estado == 'cancelado' ? 'selected' : ''}>Cancelado</option>
            </select>
          </div>
          <div style="display:flex;gap:12px;padding-top:20px;border-top:1px solid #e2e8ee;">
            <button type="submit" class="btn" style="background:#1a2e4a;color:#fff;padding:10px 22px;">
              <i class="fas fa-save me-1"></i> ${accion == 'create' ? 'Guardar documento' : 'Actualizar documento'}
            </button>
            <a href="${pageContext.request.contextPath}/documentos" class="btn btn-secondary" style="padding:10px 22px;">
