                        ...
                        <div class="sb-main">
                            ...
                            <c:if test="${sessionScope.usuarioRol == 'ENFERMERO'}">
                                <div class="rol-notice mb-3">
                                    ...
                                    Modo <strong>
                                        <fmt:message key="nav.readonly" />
                                    </strong>
                                    <fmt:message key="cita.readonly.notice" />
                                </div>
                            </c:if>
                            ...
                            <div class="page-card p-0 overflow-hidden">
                                <div class="d-flex align-items-center justify-content-between p-3"
                                    ...
                                    <div class="text-white">
                                        <h5 class="mb-0 fw-bold">
                                            ...
                                            <c:choose>
                                                <c:when test="${sessionScope.usuarioRol == 'MEDICO'}">
                                                    <fmt:message key="cita.my.title" />
                                                </c:when>
                                                <c:otherwise>
                                                    <fmt:message key="cita.title" />
                                                </c:otherwise>
                                            </c:choose>
                                        </h5>
                                        <small class="opacity-75">
                                            <c:choose>
                                                <c:when test="${sessionScope.usuarioRol == 'MEDICO'}">
                                                    <fmt:message key="cita.agenda" />
                                                </c:when>
                                                <c:when test="${sessionScope.usuarioRol == 'RECEPCIONISTA'}">
                                                    <fmt:message key="cita.manage" />
                                                </c:when>
                                                <c:otherwise>
                                                    <fmt:message key="cita.readonly.subtitle" />
                                                </c:otherwise>
                                            </c:choose>
                                        </small>
                                    </div>
                                ...
                                <div class="p-3">
                                    ...
                                    <%-- FILTROS — solo para roles que ven todas las citas --%>
                                        <c:if test="${sessionScope.usuarioRol != 'MEDICO'}">
                                            <div
                                                style="background:#e8f2ee;border-radius:12px;padding:.9rem 1rem;border:1px solid #ccddd5;margin-bottom:1rem;">
                                                <form method="get" action="${pageContext.request.contextPath}/citas"
                                                    class="d-flex gap-2 align-items-end flex-wrap">
                                                    <input type="hidden" name="accion" value="listar" />
                                                    <div>
                                                        <label
                                                            style="font-size:.76rem;font-weight:700;color:#4a6258;display:block;margin-bottom:.3rem;text-transform:uppercase;letter-spacing:.04em;">
                                                            <i class="fas fa-stethoscope me-1"></i>
                                                            <fmt:message key="cita.filter.specialty" />
                                                        </label>
                                                        <select name="filtroEspecialidad"
                                                            class="form-select form-select-sm"
                                                            style="min-width:160px;border-radius:8px;border-color:#ccddd5;">
                                                            <option value="">
                                                                <fmt:message key="cita.filter.all" />
                                                            </option>
                                                            <c:forEach var="esp" items="${especialidades}">
                                                                <option value="${esp.idEspecialidad}"
                                                                    ${filtroEspecialidad==esp.idEspecialidad.toString()
                                                                    ? 'selected' : '' }>${esp.nombre}</option>
                                                            </c:forEach>
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label
                                                            ...
                                                            <fmt:message key="cita.filter.status" />
                                                        </label>
                                                        <select name="filtroEstado" class="form-select form-select-sm"
                                                            style="min-width:150px;border-radius:8px;border-color:#ccddd5;">
                                                            <option value="">
                                                                <fmt:message key="cita.filter.all" />
                                                            </option>
                                                            <option value="PROGRAMADA" ${filtroEstado=='PROGRAMADA'
                                                                ? 'selected' : '' }>Programada</option>
                                                            <option value="CONFIRMADA" ${filtroEstado=='CONFIRMADA'
                                                                ? 'selected' : '' }>Confirmada</option>
                                                            <option value="ATENDIDA" ${filtroEstado=='ATENDIDA'
                                                                ? 'selected' : '' }>Atendida</option>
                                                            <option value="CANCELADA" ${filtroEstado=='CANCELADA'
                                                    ...
                                                    <button type="submit"
                                                        ...
                                                        <fmt:message key="cita.filter.apply" />
                                                    </button>
                                                    <a href="${pageContext.request.contextPath}/citas?accion=listar"
                                                        class="btn btn-outline-secondary btn-sm rounded-pill"
                                                        style="height:31px;line-height:1.6;">
                                                        <i class="fas fa-times me-1"></i>
                                                        <fmt:message key="cita.filter.clear" />
                                                    </a>
                                                    <span
                                                        style="font-size:.78rem;color:#4a6258;align-self:center;margin-left:auto;">
                                                        <strong>${citas.size()}</strong> resultado(s)
                                                    </span>
                                                </form>
                                            </div>
                                        </c:if>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td class="td-actions">
                                                                        <c:choose>
                                                                            ...
                                                                            <c:when
                                                                                test="${sessionScope.usuarioRol == 'MEDICO'}">
                                                                                <div class="btn-fan">
                                                                                    <c:choose>
                                                                                        <c:when
                                                                                            test="${cita.estado != 'CONFIRMADA' && cita.estado != 'ATENDIDA' && cita.estado != 'CANCELADA'}">
                                                                                            <button type="button"
                                                                                                class="fan-btn fan-edit"
                                                                                                title="Confirmar"
                                                                                        ...
                                                                                        <c:otherwise>
                                                                                            <span
                                                                                                class="fan-btn fan-edit"
                                                                                                style="opacity:.3;cursor:default;pointer-events:none;">
                                                                                                <i
                                                                                                    class="fas fa-calendar-check"></i>
                                                                                            </span>
                                                                                        </c:otherwise>
                                                                                    ...
                                                                                    <c:choose>
                                                                                        <c:when
                                                                                            test="${cita.estado != 'ATENDIDA' && cita.estado != 'CANCELADA'}">
                                                                                            <button type="button"
                                                                                                class="fan-btn fan-delete"
                                ...
                                <div id="toastContainer"
                                    style="position:fixed;top:70px;right:20px;z-index:9999;display:flex;flex-direction:column;gap:8px;max-width:320px;">
                                </div>
                                <style>
                                    .sb-toast {
                                        background: #fff;
                                    ...
                                    .sb-toast.toast-error .sb-toast-icon {
                                        color: #e74c3c;
                                    }

                                    .sb-toast.toast-warning .sb-toast-icon {
                                        color: #f39c12;
                                    }

                                    .sb-toast.toast-info .sb-toast-icon {
                                        color: #3498db;
                                    }
                                ...
                                <%-- Flash message: leer antes del script, limpiar en JSP --%>
                                    <c:if test="${not empty sessionScope.flashMessage}">
                                        <div id="flashData" data-msg="${sessionScope.flashMessage}"
                                    ...
                                    <form id="frmEstado" method="post" action="${pageContext.request.contextPath}/citas"
                                        style="display:none;">
